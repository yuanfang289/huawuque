...
var Form = {
    ...
    initWorkstage: function() {
        $.ajax({
            url: '/workstageVisit/workstagesOfUser',
            // url: '/assets/views/visitor/data/workstagesOfUser.json',
            type: 'GET',
            dataType: 'json',
            error: function(xhr) {
                UR.msg('数据获取失败请刷新重试', {
    ...
    initMeetingPlace: function() {
        $.ajax({
            ...
            success: function(res) {
                ...
                if(data){
                    var id = data[0].id;
                    var stageName = data[0].stageName;
                }
                $('#workstageId').val(id);
