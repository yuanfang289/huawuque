...
var BaseForm = {
    ...
    inputContent: function(element) {
        var $el       = $(element),
            ...
            currentPageNo = 1, oldHash;

        var opts = UR.getAttrOptions($el.data('options'));
        ...
        if(type == 'tel'){
            var $telInput = $target.find('[type="tel"]');
            var $inputParent = $telInput.parent();
            var $wrap = $telInput.closest('.form-item');
            if($wrap.hasClass('add-stateBtn')){
                return;
            }
            $wrap.addClass('add-stateBtn');
            var $mobile = $('#mobile_dummy');
            var code = $mobile.html().split(' ')[0].substring(1) || '86';
            var html = '<div class="national" national>+<em>' + code + '</em><i class="uricon-arrow-bottom ricon"></i><input type="hidden" name="nationalCode" value="' + code + '"></div>';
            $inputParent.before(html);
        }
        ...
        window.onhashchange = function(e) {
            if (location.hash == '' && oldHash == 'inputContent') {
                close();
            }
        }

        function show() {
            location.hash = oldHash = 'inputContent';
        ...
        function save(e) {
            ...
            var val, elem, stateCode;

            if (mode == 'input' || mode == 'textarea') {
                stateCode = $('[name="nationalCode"]').val();
                val  = $.trim($input.val());
                elem = $input[0];
                ...
                if (val) {
                    if($input.attr('type') == 'tel'){
                        $el.addClass('writed').text('+'+ stateCode + ' ' + val);
                    }else {
            ...
            if (mode == 'search') {
                ...
                var searchMobile  = this.getAttribute('data-mobile') || '';
                var searchCode  = this.getAttribute('data-code') || '';
                var searchName  = this.getAttribute('data-text') || '';
                elem = this;
                ...
                $hidden.val(val +  ',' + searchName + ',' + searchMobile + ',' + searchCode);
            }
        ...
        function close() {
            if (mode == 'input' || mode == 'textarea') {
                ...
                $target.find('.add-stateBtn').removeClass('add-stateBtn');
                $target.find('.national').remove();
            }
            ...
            $.ajaxPost({
                ...
                beforeSend: function () {
                    if (options.validator && $.isFunction(options.validator)) {
                        return UR.validate(post, options.validator());
                    }
