  ...
  <page-wrap class="page-user-center">
    ...
    <div v-if="!isApp" class="index-header">
      <div class="left" @click="dialog=true">
        ...
        <div class="location-list">{{stageInfo}}<!--span>·{{stageInfo[1]}}</span--></div>
      </div>
      <div class="left">
    ...
    <page-main :show="show">
          ...
          <template v-for="(item,index) in schedules">
            <div class="schedule-item" @click="schedule_detail(item)">
              ...
              <div class="schedule-content">
                ...
                <div class="schedule-location"><span v-if="item.location[0]!=''">{{item.location[0]}}</span>
                  {{item.location[1]}}
                </div>

              </div>
            </div>
            <div v-if="index<schedules.length-1||schedules.length>schedules_H" class="schedule-dividers"></div>
          </template>
        </div>
        <div v-if="schedules.length>schedules_H" class="schedule-more" @click="schedule_more()">
          <p><span class="s-m-txt">{{schedule_spread ? '收起日程' : '更多日程'}}</span><span
            :class="{'s-m-icon':true,'s-m-icon-reverse':schedule_spread}"></span></p>
        </div>
      ...
      <div v-if="managers.length>0" class="manager">
        <div class="manager-title">我的社区管家</div>
            ...
            <div class="manager-list-item">
              <a v-if="!isNaN(item.userId)" :href="'/user/userDetail?userId='+item.userId">
                <div class="manager-img">
                  <img :src="item.image1+'?imageMogr2/thumbnail/!120x120r/gravity/Center/crop/120x120/format/'+isWebp+'/quality/90'" onerror="this.src='https://assets.urwork.cn/v2/assets/images/pic_def.f71a172.png';this.onerror=null;"/>
                </div>
              </a>
                ...
                <div class="manager-info">
                  ...
                  <div class="manager-item-title">{{item.job}}</div>
                </div>
        ...
        <div class="n-m-list">
          <template v-for="item in meetingRoom">
            <div class="n-m-item"
              ...
              <div class="n-m-img"><img :src="item.pic+'?imageMogr2/thumbnail/!324x243r/gravity/Center/crop/324x243/format/'+isWebp+'/quality/90'"
        ...
        <div class="dialog">
          ...
          <div class="list">
            <template v-for="item in myWorkstage">
              <div :class="{'item':true,'item-select':item.selected}" @click="choseWorkstage(item.id)">{{item.stageName}}</div>
            </template>
          </div>
        </div>
      </div>
      ...
      schedules_data(data){
        data.map(item => {
          ...
          if (item.dayType == 3||item.dayType == 0) {
            item.date = (date_t.getMonth() + 1 > 9 ? (date_t.getMonth() + 1) : ("0" + (date_t.getMonth() + 1))) + "月" + (date_t.getDate() > 9 ? date_t.getDate() : ("0" + date_t.getDate())) + '日';
          } else {
            item.date = item.dayType == 1 ? '今天' : '明天';
          }
          item.sType = item.typeName == '活动' ? 2 : (item.typeName == '会议' ? 1 : (item.typeName == '访客'&&item.title=='预约参观'?4:3));
...
<style lang="scss" scoped="">
  ...
  .m-main {
    ...
    .m-main-list {
      width: 100%;
      padding: 0.6rem 0.38rem 0.8rem 0.40rem;
    ...
    .schedule-more {
      ...
      p {
        ...
        .s-m-icon {
          ...
          height: 0.32rem;
          display: inline-block;
          background: url("https://assets.urwork.cn/image/v3/down-more.png") no-repeat;
          background-size: 100% 100%;
        }
        .s-m-icon-reverse {
