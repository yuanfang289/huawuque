  ...
  <page-wrap class="page-user-center">
    ...
    <div v-if="!isApp" class="index-header">
      <div class="left" @click="showSelCity=true">
        ...
        <div class="location-list">{{cityName}}<!--span>·{{stageInfo[1]}}</span--></div>
      </div>
      <div class="left">
    ...
    <page-main :show="show">
      <div class="m-main">
        <div class="m-main-list">
          <div v-for="item in modules" class="m-main-item" @click="gotoFun(item)">
            <div class="m-img"><img :src="item.img"/></div>
      ...
      <div class="excellent">
        ...
        <div class="e-title">
          <div class="e-title-txt">发现气质社区</div>
          ...
          <div class="e-city-list">
            <div class="e-city-list-content">
              <template v-for="item in hotCity">
                <div @click="getEWorkstages(item.cityCode)"
                     :class="{'e-city-item':true,'e-city-item-sel':item.selected}">{{item.cityName}}
                </div>
              </template>
            </div>
          </div>
        ...
        <div class="e-w-list">
          ...
          <div v-for="item in excellentWorkstages" class="e-w-list-item"
               @click=" gotoFun({url:'/workstage/detail?id='+item.id,appUrl:''})">
            <div class="e-w-list-item-img"><img :src="item.workstageImage+'?imageMogr2/thumbnail/!324x243r/gravity/Center/crop/324x243/format/'+isWebp+'/quality/90'" onerror="this.src='https://image.urwork.cn/57a6da91-6bf1-4366-8036-c693cff34a74.jpg';this.onerror=null;"></div>
            <div class="e-w-list-item-name"> {{item.stageName}}</div>
            <div class="e-w-list-item-addr">{{item.address}}</div>

            <div class="e-w-list-item-detail">
              <div class="e-w-list-item-price no-price" v-if="!item.openStation">价格致电详询</div>
              <div class="e-w-list-item-price" v-if="item.openStation">￥{{item.openStation}}<span
                class="e-w-list-item-price-title">/月</span></div>
        ...
        <div class="e-w-more" @click=" gotoFun({url:'/v3/workStage'+'?currentLongitude='+longitude+'&currentLatitude='+latitude,appUrl:'goodStage'+'?longitude='+longitude+'&latitude='+latitude})">更多</div>

      </div>

      <div class="form">

        <div class="form-title">您想在哪里办公？</div>

        <div class="form-content">
          <div class="form-item" @click="showSelArea=true">
            <input readonly="readonly" placeholder="选择地点" :value="areaName" type="text"/>
            <span class="location-icon"></span>
          </div>

          <div class="form-item">
            <input type="txt" v-model="formData.number" maxlength="5" placeholder="公司人数"/>
            <span class="people-icon"></span>
          </div>

          <div class="form-item-phone">
            <div class="zone">
              <select style="  -webkit-appearance: none;-moz-appearance: none;appearance: none;background-color: white"
                      v-model="formData.nationalCode">
                <option :value="item" style="background-color: #fff;" v-for="item in nationalCodes">+{{item}}</option>
              </select>
              <span></span>
            </div>
            <div class="user-contact">
              <input type="text" maxlength="11" v-model="formData.phone" class="input-phone" placeholder="手机号码"/>
            </div>
          </div>
        </div>

        <div class="form-sbumit" @click="form_submit"><span>提交</span></div>
      </div>

      <div v-if="managers.length>0" class="manager">
        <div class="manager-title">招商顾问</div>
          ...
          <template v-for="(item,index) in managers">
            <div class="manager-list-item">
              <a v-if="!isNaN(item.userId)" :href="'/user/userDetail?userId='+item.userId">
                ...
                <img :src="item.headImage+'?imageMogr2/thumbnail/!120x120r/gravity/Center/crop/120x120/format/'+isWebp+'/quality/90'" onerror="this.src='https://assets.urwork.cn/v2/assets/images/pic_def.f71a172.png'"/>
              </div>
              </a>
              ...
              <div class="manager-contact">
                <div class="manager-contact-info">
                  ...
                  <div v-if="!isNaN(item.userId)"  @click="gotoFun({url:'',appUrl:'conversation?targetUserId='+item.userId})" class="manager-contat-msg"></div>
        ...
        <div class="n-m-list">
          <template v-for="item in meetingRoom">
            <div class="n-m-item"
              ...
              <div class="n-m-img"><img :src="item.pic?item.pic+'?imageMogr2/thumbnail/!324x243r/gravity/Center/crop/324x243/format/'+isWebp+'/quality/90':'1'"
                                        :onerror="'this.src=\'https://image.urwork.cn/7f95910f-5096-44dd-a86c-4037d5d257bf.jpg'+'?imageMogr2/thumbnail/!324x243r/gravity/Center/crop/324x243/format/'+isWebp+'/quality/90\';this.onerror=null'"/>
              </div>
              <div class="n-m-desc">
                ...
                <div class="n-m-price">
                  <span class="n-m-price-val">￥{{item.rentalPrice}}</span><span class="n-m-price-name">/半小时</span>
                </div>
              </div>
            </div>
          </template>
        </div>
        <div class="n-m-more" @click=" gotoFun({url:'/rentMeetingRoom/list',appUrl:'meetingRoom'})">更多</div>
      </div>
      ...
      <select-area :show="showSelArea" @sel-area-close="showSelArea=false;" @select-area="getSelArea"></select-area>
...
<script>
  ...
  export default {
    ...
    data: () => {
      return {
        ...
        modules: [
          ...
          {name: '访客预约', img: 'https://assets.urwork.cn/image/v3/visitor.png', url: '/v2/visit', appUrl: ''},
          {
            name: '优质社区',
          ...
          {name: '管理输出', img: 'https://assets.urwork.cn/image/v3/manager-out.png', url: '/urworkCity', appUrl: ''},
          {name: '更多服务', img: 'https://assets.urwork.cn/image/v3/more-service.png', url: '/v3/applyList', appUrl: ''},
        ],
        hotCity: [
    ...
    methods: {
      ...
      getData(){
        apiIndexNew({
          ...
          this.excellentWorkstages = this.excellentWorkstages_data(res.excellentWorkstages);
          let hotCity_arr = res.hotCity;
          if (this.longitude && this.latitude) {
            hotCity_arr.unshift({cityCode: "", cityName: "附近", cityNameEn: "fujin"});
          }
    ...
    created(){
      ...
      this.modules[3].appUrl='goodStage?longitude='+this.longitude+'&latitude='+ this.latitude;
...
<style lang="scss" scoped="">
  ...
  .index-header {
    ...
    .left {
      ...
      .location-icon {
        ...
        background: url("https://assets.urwork.cn/image/v3/location.png") no-repeat;
  ...
  .m-main {
    ...
    .m-main-list {
      width: 100%;
      padding: 0.60rem 0.38rem 0.80rem 0.40rem;
    ...
    .form-content {
      ...
      .form-item-phone {
        ...
        .zone {
          ...
          span {
            ...
            height: 0.20rem;
            //margin-left: 0.12rem;
            background: url("https://assets.urwork.cn/image/v3/arrow_down.png") no-repeat;
            background-size: 100% 100%;
          }
        }
        .user-contact {
      ...
      .form-item {
        ...
        .people-icon {
          ...
          width: 0.4rem;
          height: 0.4rem;
          background: url("https://assets.urwork.cn/image/v3/people.png") no-repeat;
          background-size: 100% 100%;
        }
      }
    }
        ...
        .manager-contact {
          ...
          .manager-contact-info {
            ...
            .manager-contat-tel {
              width: 0.48rem;
              height: 0.48rem;
              background: url("https://assets.urwork.cn/image/v3/phone.png") no-repeat;
              background-size: 100% 100%;
              margin-right: 0.6rem;
            }
            .manager-contat-msg {
              width: 0.48rem;
              height: 0.48rem;
              background: url("https://assets.urwork.cn/image/v3/msg.png") no-repeat;
              background-size: 100% 100%;
            }
          }
        }
